...
export function isActivePath(pathname: string | null, href: string, exact?: boolean, anchor?: string) {
  if (!pathname) return false;
  // Los items con anchor (#seccion) apuntan a la misma ruta base: solo deben
  // marcarse activos cuando el pathname coincide exactamente, nunca en
  // subrutas tipo /docente/talleres (que tendrían su propio item).
  if (anchor) return pathname === href;
  ...
  return (
    <div className="flex h-full flex-col bg-white">
      ...
      <nav className="flex-1 space-y-1 overflow-y-auto px-3 py-4">
        ...
        {items.map((item) => {
          const active = isActivePath(pathname, item.href, item.exact, item.anchor);
...
export const ADMIN_NAV: SidebarItem[] = [
  ...
  { href: "/admin/docentes", label: "Registrar docente", icon: UserPlus },
  { href: "/admin/sesiones", label: "Sesiones", icon: Presentation },
...
export const DOCENTE_NAV: SidebarItem[] = [
  ...
  { href: "/docente/talleres", label: "Talleres", icon: Library },
  { href: "/docente/materiales", label: "Materiales", icon: FolderOpen },
];
