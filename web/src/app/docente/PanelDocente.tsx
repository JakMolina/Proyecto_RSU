...
export function PanelDocente({ docente, sesiones, progreso, certificado }: Props) {
  ...
  return (
    <div className="space-y-8">
      ...
      <section className="hero-gradient relative overflow-hidden rounded-3xl p-8 text-white shadow-lg md:p-12">
        ...
        <div className="relative">
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            <Link href="/verificar" className="btn bg-white text-brand-700 hover:bg-blue-50">
            ...
            {certificado?.tiene_pdf ? (
              <div className="mt-6 flex flex-1 flex-col items-center justify-center rounded-2xl bg-gradient-to-br from-emerald-50 to-blue-50 p-6 text-center">
                ...
                <div className="mt-5 flex flex-wrap items-center justify-center gap-2">
                  ...
                  <Link href={`/verificar`} className="btn-ghost">Verificar online</Link>
                </div>
              </div>
            ) : progreso.cumple ? (
      ...
      {/* Timeline de talleres */}
      <section id="talleres" style={{ scrollMarginTop: 80 }}>
        <h2 className="mb-4 text-xl font-bold text-slate-900">Mis talleres</h2>
        <div className="grid gap-4 md:grid-cols-2">
          {sesiones.map((s, i) => {
            const info = TALLERES_INFO[s.taller_numero] ?? { emoji: "📘", color: "from-slate-400 to-slate-500", tools: "" };
            return (
              <div
                key={s.id}
                className={`card animate-fade-up-delay-${Math.min(i, 3)} overflow-hidden p-0`}
              >
                <div className="flex">
                  ...
                  <div className="flex-1 p-4">
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        <p className="text-xs font-medium text-slate-400">Taller {s.taller_numero}</p>
                        <h3 className="font-semibold text-slate-900">{s.nombre}</h3>
                      </div>
                      {s.asistio ? (
                        <span className="badge bg-emerald-100 text-emerald-700">Asistió ✓</span>
                      ) : (
                        <span className="badge bg-slate-100 text-slate-500">Pendiente</span>
                      )}
                    </div>
                    <p className="mt-2 text-xs text-slate-500">
                      {new Date(s.fecha + "T00:00").toLocaleDateString("es-PE", { weekday: "long", day: "numeric", month: "long" })} · {s.hora_inicio}–{s.hora_fin}
                    </p>
                    {s.registrado_en && (
                      <p className="mt-1 text-xs text-emerald-600">Registrado: {new Date(s.registrado_en).toLocaleString("es-PE")}</p>
                    )}
                    {info.tools && <p className="mt-2 text-xs font-medium text-brand-700">{info.tools}</p>}
                  </div>
                </div>
              </div>
            );
          ...
          {!sesiones.length && (
            <p className="text-slate-500">Aún no hay sesiones programadas.</p>
          )}
        </div>
      </section>
