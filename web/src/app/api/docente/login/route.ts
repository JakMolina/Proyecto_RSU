...
export async function POST(req: Request) {
  ...
  if (!/^\d{8}$/.test(dniLimpio)) {
    return noStore(NextResponse.json({ error: "DNI inválido (8 dígitos)." }, { status: 422 }));
  }
  ...
  if (!part) return noStore(NextResponse.json({ error: "Este DNI no está registrado. Regístrate primero." }, { status: 404 }));
