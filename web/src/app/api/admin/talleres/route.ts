...
export async function GET() {
  ...
  return noStore(NextResponse.json({ talleres: out }));
}
...
export async function POST(req: Request) {
  ...
  if (!session_id) return noStore(NextResponse.json({ error: "session_id obligatorio" }, { status: 422 }));
  if (!nombre) return noStore(NextResponse.json({ error: "Nombre obligatorio" }, { status: 422 }));
  if (!Number.isInteger(orden) || orden < 1) {
    return noStore(NextResponse.json({ error: "Orden inválido (entero ≥ 1)" }, { status: 422 }));
  }

  const sb = supabaseService();
  const { data: sesion } = await sb.from("sesiones").select("id").eq("id", session_id).maybeSingle();
  if (!sesion) return noStore(NextResponse.json({ error: "La sesión indicada no existe" }, { status: 422 }));
