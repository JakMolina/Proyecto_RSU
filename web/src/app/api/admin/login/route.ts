...
export async function POST(req: Request) {
  ...
  if (!email || !password) return NextResponse.json({ error: "faltan credenciales" }, { status: 422 });
  ...
  if (error || !authed.session) return NextResponse.json({ error: "Credenciales inválidas" }, { status: 401 });

  const uid = authed.user!.id;
  const { data: admin } = await supabaseService().from("admins").select("email").eq("id", uid).maybeSingle();
  if (!admin) return NextResponse.json({ error: "Sin permisos de administrador" }, { status: 403 });
