...
export async function POST(req: Request) {
  ...
  if (!validarDni(dni)) return NextResponse.json({ error: "DNI inválido (8 dígitos)." }, { status: 422 });
  if (!nombres || !apellidos) return NextResponse.json({ error: "Nombres y apellidos obligatorios." }, { status: 422 });
  if (!validarWhatsapp(whatsapp)) return NextResponse.json({ error: "WhatsApp inválido. Ej: +51 987654321." }, { status: 422 });

  const sb = supabaseService();
  const { data: exist } = await sb.from("participantes").select("id").eq("dni", dni).maybeSingle();
  if (exist) return NextResponse.json({ error: "Ya existe un participante con ese DNI." }, { status: 409 });
...
export async function PATCH(req: Request) {
  ...
  if (body.dni !== undefined) {
    ...
    if (dup) return NextResponse.json({ error: "Ese DNI ya lo usa otro participante." }, { status: 409 });
    patch.dni = dni;
  }
  if (body.nombres !== undefined) {
    const n = String(body.nombres).trim();
    if (!n) return NextResponse.json({ error: "Nombres obligatorio." }, { status: 422 });
    patch.nombres = n;
  }
  if (body.apellidos !== undefined) {
    const a = String(body.apellidos).trim();
    if (!a) return NextResponse.json({ error: "Apellidos obligatorio." }, { status: 422 });
    patch.apellidos = a;
  }
  if (body.whatsapp !== undefined) {
    const w = String(body.whatsapp).trim();
    if (!validarWhatsapp(w)) return NextResponse.json({ error: "WhatsApp inválido. Ej: +51 987654321." }, { status: 422 });
    patch.whatsapp = w;
  }
